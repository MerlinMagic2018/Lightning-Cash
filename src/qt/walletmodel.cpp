...
bool WalletModel::createBees(int beeCount, bool communityContrib, QWidget *parent, double beePopIndex) {
    ...
    if ((Params().GetConsensus().variableBeecost) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().variableForkBlock)) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().remvariableForkBlock))) {
	   // LogPrintf("OK \n");
	    if (!wallet->CreateBeeTransaction3(beeCount, wtxNew, reservekeyChange, reservekeyHoney, honeyAddress, communityContrib, strError, Params().GetConsensus())) {
    ...
    if (beePopIndex > 90)
        questionString.append("<br /><span style='color:#aa0000;'><b>WARNING:</b> Global Index is high and bees may not be profitable. If you still want to create bees, know that they will also cost 2x more than normal price. Please ensure you understand the consequences before proceeding.</span><br />");
