...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns a new LightningCash Gold address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"address\"    (string) The new lightningcash_gold address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current LightningCash Gold address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account lightningcash_gold address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new LightningCash Gold address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lightningcash_gold address to be associated with an account.\n"
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid LightningCash Gold address");
    }
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lightningcash_gold address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a lightningcash_gold address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The lightningcash_gold address to send to.\n"
            ...
            "                             The recipient will receive less lightningcash_gold than you enter in the amount field.\n"
...
// LightningCash Gold: Hive: Get current bee cost
...
// LightningCash Gold: Hive: Create bee(s)
UniValue createbees(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "3. \"honey_address\"        (string, optional) The LTNCG address to receive rewards for blocks mined by bee(s) created in this transaction.\n"
			"4. \"change_address\"       (string, optional, default pool address) The LTNCG address to receive the change.\n"
            "\nResult:\n"
            "\"txid\"                    (string) The transaction id.\n"
            "\nExamples:\n"
...
// LightningCash Gold: Hive: Return BCT tx id for a honey transaction in this wallet
...
// LightningCash Gold: Hive: Get wallet's own hive info
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The lightningcash_gold address\n"
            "      amount,                 (numeric) The amount in " + CURRENCY_UNIT + "\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lightningcash_gold address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lightningcash_gold address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a lightningcash_gold address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The lightningcash_gold address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The lightningcash_gold address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less lightningcash_gold than you enter in their corresponding amount field.\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "Each key is a LightningCash Gold address or hex-encoded public key.\n"
            ...
            "2. \"keys\"                         (string, required) A json array of lightningcash_gold addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"                  (string) lightningcash_gold address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start lightningcash_goldd with -deprecatedrpc=addmultisigaddress).\n"
            "        clients should transition to the new output api before upgrading to v0.17.\n"
            "\"address\"                         (string) A lightningcash_gold address associated with the keys.\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart lightningcash_goldd with -deprecatedrpc=addwitnessaddress.\n"
...
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            // LightningCash Gold: Hive: Sent transactions are never honey
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                // LightningCash Gold: Hive: Indicate whether this is honey (hive block coinbase tx)
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    // LightningCash Gold: Hive: Include ishoney in documentation
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The lightningcash_gold address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The lightningcash_gold address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The lightningcash_gold address involved in the transaction\n"
...
// LightningCash Gold: Hive: Callback to set the wallet back to unlocked only for hive on walletpassphrase timeout
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending lightningcash_gold\n"
            "\nArguments:\n"
	...
	// LightningCash Gold: Hive: Support locked wallets
...
// LightningCash Gold: Hive: Unlock wallet for hiving only (Prevent trivial sendmoney attack if user OS account compromised)
UniValue walletpassphrasehiveonly(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
			...
			"private keys such as sending lightningcash_gold, is not enabled and will require you to run\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending lightningcash_gold\n"
    ...
    StartShutdown();
    return "wallet encrypted; LightningCash Gold server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending lightningcash_gold.\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of lightningcash_gold addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the lightningcash_gold address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid LightningCash Gold address: ") + input.get_str());
            }
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The lightningcash_gold address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid lightningcash_gold address");
            }
...
{ //  category              name                        actor (function)           argNames
    //  --------------------- ------------------------    -----------------------  ----------
    { "wallet",             "getbeecost",               &getbeecost,               {"height"} },                                            // LightningCash Gold: Hive: Get bee cost for given height (defaults to tipheight)
    { "wallet",             "createbees",               &createbees,               {"bee_count","community_contrib","honey_address"} },     // LightningCash Gold: Hive: Create bee(s)
    { "wallet",             "gethiveinfo",              &gethiveinfo,              {"include_dead","min_honey_confirms"} },                 // LightningCash Gold: Hive: Get current hive info
    { "wallet",             "getnetworkhiveinfo",       &getnetworkhiveinfo,       {"include_graph"} },                                     // LightningCash Gold: Hive: Get current bee populations across whole network
    { "wallet",             "getbeecreationtxid",       &getbeecreationtxid,       {"honey_txid"} },                                        // LightningCash Gold: Hive: Return BCT tx id for a honey transaction in this wallet
